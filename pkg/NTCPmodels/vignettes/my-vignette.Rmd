...
library(diagram)
openplotmat(main = "")
elpos <- coordinates (c(1, 1,1,1,1), mx = 0, my = 0)
#elpos[,2]<-seq(1,0,length.out = 5)
straightarrow(from = elpos[1, ], to = elpos[2, ], lty = 1, lcol = 1,arr.pos = 0.6)
straightarrow(from = elpos[2, ], to = elpos[3, ], lty = 1, lcol = 1,arr.pos = 0.6)
straightarrow(from = elpos[3, ], to = elpos[4, ], lty = 1, lcol = 1,arr.pos = 0.6)

bentarrow(from = elpos[4, ], to = elpos[5, ],lty=1,lcol=1)
...
labels<-c("(1) 3-D CT scans",
 ...
 cex<-0.7
 textrect (elpos[1, ], 0.15, 0.05, lab = labels[1], cex = cex)
 textrect (elpos[2, ], 0.1, 0.05, lab = labels[2], cex = cex)
 textrect (elpos[3, ], 0.2, 0.05, lab = labels[3], cex =cex)
 textrect (elpos[4, ], 0.2, 0.05, lab = labels[4], cex = cex)
 textrect (elpos[5, ], 0.2, 0.05, lab = labels[5], cex = cex)
```
...
str(DVH[1:2])
```
If EQD2 transformation is neccesary, then make sure you have the fractionations of all the patients stored in a vector. For the inbuit data this is store in the `fractionation` object, see below.
